...
class Cart extends AbstractController
{
    ...
    public function view($action=NULL){
        ...
        if($_POST['submit']=="Submit Payment"){
            unset($_POST['submit']);
            self::checkout();
        }
...
<script type='text/javascript'>
	...
	return discount;
        ...
        foreach($_REQUEST as $course=>$data){ //handles group order submission
            if(is_array($data)){
                if($data['quantity']){
                    $_POST['type'][]=$course;
                    $group = 1;
		...
		if(!$_REQUEST['type'] && !$_REQUEST['groupon'] && !$group){
			$_POST['type'][] = "cert_1";
		...
		Loader::model('attribute/categories/user');		
			...
			if($_POST['type'] || $_GET['type'] && !$_REQUEST['add-course']){ //posting from front page
				if(!$_POST){
					$_POST = $_GET;
				}
		...
		if (!$e->has() && !$added_course) { //there are no validation errors. Attempt payment
			...
			if ($uID == 1 && $data['cc_type'] == "admin" || self::$test_mode == true || $uID == self::$testID){ // This code allows us to fake the paypal payment in the event of an admin override
				...
				if(!$db->getOne($q)){
					...
					header("Location: ". Config::get('app.thanks_url'), TRUE, 303); //forward user to thank you page.
					die();
				}
			}
			...
			if ($paypal_results_array['ACK'] != "Success" &&  $paypal_results_array['ACK'] != "SuccessWithWarning"){ // There was an error
				...
				email::send_receipt($uID, $record_transaction,$data['discount']); //send receipt email
					...
					if(!$data['groupon']){
						...
						header("Location: ".Config::get('concrete.seo.canonical_url')."/member", TRUE, 303); 
					}
					die();
			}
